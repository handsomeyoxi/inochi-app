...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q,
      (snap) => { setFsProducts(snap.docs.map(d => ({ ...d.data(), _docId: d.id }))); setFsLoading(false); },
      () => setFsLoading(false)
    );
    return () => unsub();
  }, [store.name]);
  ...
  const itemsTotal  = regularProducts.reduce(
    (sum, p) => sum + p.specialPrice * (itemQtys[p._docId] ?? 0), 0
  );
  ...
  /* ── 顯示 header 資訊 ── */
  const displayStock = boxProduct?.stock ?? store.stock_quantity ?? '—';
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white px-4 py-3 flex items-center gap-3 shadow-sm shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">
            截止 {displayDeadline} · 剩 {displayStock} 份
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {fsLoading ? (
          <div className="text-center text-gray-400 py-16 text-sm">載入商品中…</div>
        ) : (
          <>
            {/* 驚喜包 */}
            <div className="m-4">
              ...
              <div className="bg-gradient-to-br from-orange-400 via-amber-400 to-yellow-400 rounded-3xl p-5 shadow-lg">
                <div className="text-center mb-3">
                  ...
                  <div className="flex items-baseline justify-center gap-2 mt-1">
                    ...
                    {boxProduct && (
                      <span className="text-white/60 text-sm line-through">${boxProduct.originalPrice}</span>
                    )}
                    {boxProduct && boxProduct.originalPrice > 0 && (
                      <span className="bg-white/25 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                        -{Math.round((1 - boxPrice / boxProduct.originalPrice) * 100)}%
                      </span>
                    )}
                  </div>
                  {boxProduct && (
                    <div className="text-white/60 text-xs mt-1">剩餘庫存：{boxProduct.stock} 份</div>
                  )}
                </div>
              ...
              <div className="mx-4 mb-4">
                ...
                <div className="bg-white rounded-2xl shadow-sm divide-y divide-gray-50">
                  {regularProducts.map(p => {
                    const qty = itemQtys[p._docId] ?? 0;
                    return (
                      <div key={p._docId} className="flex items-center px-4 py-3.5 gap-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button onClick={() => adjustItem(p._docId, -1)}
      ...
      <div className="shrink-0 bg-white border-t border-gray-100 px-4 py-3 shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowConfirm(true)} disabled={total === 0 || ordering}
